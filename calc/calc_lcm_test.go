...
func TestLcm(t *testing.T) {
	testCases := []struct {
		...
		{"LCM Calculation with Large Numbers", 1000000, 1000001, 1000000000000},
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if result != tc.expected {
				t.Errorf("Expected LCM(%d, %d) to be %d, but got %d", tc.a, tc.b, tc.expected, result)
			} else {
				t.Logf("LCM(%d, %d) returned the expected result: %d", tc.a, tc.b, result)
			}
		})
	}
}